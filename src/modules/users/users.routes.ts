...
import { setServiceCategorySchema, updateMeSchema } from './users.validator';
...
export const createUsersModule = (): Router => {
   ...
   *       content:
   *         application/json:
   *           schema: { $ref: '#/components/schemas/SetServiceCategoryRequest' }
   *     responses:
   *       200:
   *         description: OK
   *         content:
  ...
  router.patch(
    '/me/service-category',
    requireAuth,
    validate({ body: setServiceCategorySchema }),
    controller.setServiceCategory
  );
